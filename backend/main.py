...
# =============================================================================
# This endpoint fetches today's MLB schedule from the free MLB Stats API and
# enriches each probable pitcher with their stats from our database AND live
# career/season data from the API. This powers the "Today's Matchups" page.
#
# Data flow:
# 1. GET mlb schedule for today → list of games with probable pitchers
# 2. For each pitcher, look up stats in our local DB (current season)
# 3. Also fetch career + prior season stats from the MLB Stats API
# 4. Return game objects with both pitchers' full stat profiles


@app.get("/matchups/today", response_model=ApiResponse)
async def get_todays_matchups():
    """
    Fetch today's MLB pitching matchups with full pitcher stats.

    This endpoint combines data from TWO sources:

    1. MLB Stats API (free, no key needed):
       - Today's schedule with probable pitchers
       - Career stats for each pitcher
       - Prior season stats (e.g., 2025) for pitchers without current-year data

    2. Our local Neon database:
       - Current season stats (from the pitchers table)
       - These match what you see on the main Pitchers tab

    The result is a list of game objects, each containing:
    - Game metadata (time, venue, teams, records)
    - Away pitcher with stats across multiple time periods
    - Home pitcher with stats across multiple time periods

    Returns:
        ApiResponse with list of matchup objects
    """
    ...
    try:
        # -----------------------------------------------------------------
        # Step 1: Fetch today's schedule from the MLB Stats API.
        # The 'hydrate' parameter tells the API to include probable pitcher
        # info nested inside each game's team objects. Without it, we'd only
        # get team names and game times.
        # -----------------------------------------------------------------
        ...
        # Collect all unique pitcher MLB IDs so we can batch-fetch their stats.
        # Using a dict to map mlb_id → pitcher name for easy lookup later.
        pitcher_ids = {}
        ...
        for game in raw_games:
            for side in ['away', 'home']:
                ...
                if pp:
                    pitcher_ids[pp['id']] = pp['fullName']

        # -----------------------------------------------------------------
        # Step 2: Look up each pitcher in our local database.
        # Our 'pitchers' table stores the same stats shown on the main
        # Pitchers tab. We match by name since the DB doesn't store MLB IDs.
        # -----------------------------------------------------------------
        db = get_db()
        local_pitcher_stats = {}
        if pitcher_ids:
            # Fetch ALL pitchers from our DB and match by name.
            # This is fast because the pitchers table is small (~500 rows).
            query = pitchers.select()
            rows = await db.fetch_all(query)
            # Build a name → stats lookup dict (lowercase for case-insensitive matching)
            db_by_name = {}
            for row in rows:
                db_by_name[row['name'].lower()] = dict(row._mapping)

            for mlb_id, name in pitcher_ids.items():
                match = db_by_name.get(name.lower())
                if match:
                    local_pitcher_stats[mlb_id] = match

        # -----------------------------------------------------------------
        # Step 3: Fetch career + season stats from the MLB Stats API.
        # We request BOTH career totals and the current season in one call.
        # The API returns an array of stat groups (one per type requested).
        #
        # Why career stats? At the start of a new season (like Opening Day),
        # current-year stats are empty. Career numbers give context for who
        # a pitcher is even before they throw a pitch this year.
        # -----------------------------------------------------------------
        current_year = datetime.now().year
        ...
        if pitcher_ids:
            # The MLB Stats API accepts comma-separated IDs for batch requests.
            # We request three stat types in one call to minimize HTTP requests:
            #   - season (current year): What they've done THIS year
            #   - season (prior year):   What they did LAST year
            #   - career:                Lifetime totals
            ids_str = ','.join(str(pid) for pid in pitcher_ids.keys())
            try:
                data = statsapi.get('people', {
                    'personIds': ids_str,
                    'hydrate': f'stats(group=[pitching],type=[season,career],season={current_year})'
                })
                ...
                for person in data.get('people', []):
                    ...
                    for stat_group in person.get('stats', []):
                        stype = stat_group.get('type', {}).get('displayName', '')
                        splits = stat_group.get('splits', [])
                        if splits:
                            raw = splits[0].get('stat', {})
                            # Convert string numbers to floats for consistent JSON
                            parsed = _parse_pitching_stats(raw)
                            if stype == 'career':
                                api_pitcher_stats[pid]['career'] = parsed
                            elif stype == 'season':
                                api_pitcher_stats[pid]['season_current'] = parsed

                # Separate call for prior season stats (the hydrate param only
                # accepts one season value, so we need a second request).
                data_prior = statsapi.get('people', {
                    'personIds': ids_str,
                    'hydrate': f'stats(group=[pitching],type=[season],season={prior_year})'
                })
                for person in data_prior.get('people', []):
                    pid = person['id']
                    if pid not in api_pitcher_stats:
                        continue
                    for stat_group in person.get('stats', []):
                        splits = stat_group.get('splits', [])
                        if splits:
                            raw = splits[0].get('stat', {})
                            api_pitcher_stats[pid]['season_prior'] = _parse_pitching_stats(raw)

            except Exception as e:
                # If the MLB API call fails, we still have local DB stats.
                # Log the error but don't fail the whole endpoint.
                print(f"Warning: MLB Stats API call failed: {e}")

        # -----------------------------------------------------------------
        # Step 4: Build the response — one object per game.
        # Each game includes both pitchers with all their stat periods.
        # -----------------------------------------------------------------
        for game in raw_games:
            away_team = game.get('teams', {}).get('away', {})
            home_team = game.get('teams', {}).get('home', {})

            def build_pitcher_obj(team_data, side_label):
                """
                Assemble a pitcher object combining local DB + MLB API stats.

                Returns a dict with:
                - Pitcher identity (name, team, age, throws)
                - db_stats: Current season from our database (same as Pitchers tab)
                - season_current: Current year from MLB API
                - season_prior: Last year from MLB API
                - career: Career totals from MLB API
                """
                ...
                mlb_id = pp['id']
                api_stats = api_pitcher_stats.get(mlb_id, {})
                db_stats = local_pitcher_stats.get(mlb_id)

                return {
                    'name': pp['fullName'],
                    'team': team_data.get('team', {}).get('name', ''),
                    'mlb_id': mlb_id,
                    'age': api_stats.get('age'),
                    'throws': api_stats.get('throws', ''),
                    'db_stats': db_stats,
                    'season_current': api_stats.get('season_current') or None,
                    'season_prior': api_stats.get('season_prior') or None,
                    'career': api_stats.get('career') or None,
                }

            # Parse the game time from ISO format → readable local time.
            # The API returns UTC (e.g., "2026-03-30T20:10:00Z").
            ...
            games_data.append({
                ...
                'away': {
                    'team': away_team.get('team', {}).get('name', ''),
                    'record': away_team.get('leagueRecord', {}),
                    'pitcher': build_pitcher_obj(away_team, 'away'),
                },
                'home': {
                    'team': home_team.get('team', {}).get('name', ''),
                    'record': home_team.get('leagueRecord', {}),
                    'pitcher': build_pitcher_obj(home_team, 'home'),
                },
            })

        return ApiResponse(
            code=200,
            message=f"Found {len(games_data)} game(s) scheduled for today",
            data=games_data,
        )
        ...
        return ApiResponse(
            code=502,
            message=f"Failed to fetch today's schedule: {str(e)}",
            data=[],
        )


def _parse_pitching_stats(raw: dict) -> dict:
    """
    Normalize pitching stats from the MLB Stats API into a consistent format.

    The MLB API returns some values as strings (e.g., ERA "3.45", IP "254.2")
    and others as ints. This helper converts everything to numbers so the
    frontend can display and compare them without type-checking.

    The stat names here match what we use in the PitcherTable component,
    making it easy for the frontend to display them consistently.

    Args:
        raw: Dict of raw stat values from the MLB Stats API

    Returns:
        Dict with normalized stat names and numeric values
    """
    ...
    ip = safe_float(raw.get('inningsPitched', 0))

    return {
        'wins': safe_int(raw.get('wins', 0)),
        'losses': safe_int(raw.get('losses', 0)),
        'era': safe_float(raw.get('era', 0)),
        'whip': safe_float(raw.get('whip', 0)),
        'innings_pitched': ip,
        'hits_allowed': safe_int(raw.get('hits', 0)),
        'earned_runs': safe_int(raw.get('earnedRuns', 0)),
        'walks': safe_int(raw.get('baseOnBalls', 0)),
        'strikeouts': safe_int(raw.get('strikeOuts', 0)),
        'home_runs_allowed': safe_int(raw.get('homeRuns', 0)),
        'saves': safe_int(raw.get('saves', 0)),
        'games_played': safe_int(raw.get('gamesPlayed', 0)),
        'games_started': safe_int(raw.get('gamesStarted', 0)),
        # Computed rates — calculate here so the frontend doesn't have to.
        # These match the computed stats from our /pitchers/computed endpoint.
        'k_per_9': round((safe_int(raw.get('strikeOuts', 0)) / ip) * 9, 2) if ip > 0 else 0,
        'bb_per_9': round((safe_int(raw.get('baseOnBalls', 0)) / ip) * 9, 2) if ip > 0 else 0,
        'hr_per_9': round((safe_int(raw.get('homeRuns', 0)) / ip) * 9, 2) if ip > 0 else 0,
    }
